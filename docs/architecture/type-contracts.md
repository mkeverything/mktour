...
2. `src/server/zod/` is the only source of truth for domain schemas and types.
3. all client domain types must be inferred/imported from canonical zod schemas.
4. tRPC `.input()` and `.output()` must reference canonical schemas from `src/server/zod/`.
...
- prefer table-derived schemas in `src/server/zod/*`.
- for non-table domain payloads (filters, views, command payloads), define dedicated zod schemas in `src/server/zod/*` and export inferred types.
...
- do router inputs/outputs import schemas from `src/server/zod/`?
